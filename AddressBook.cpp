...
/*void transverseList(string name, int number, int userResponse) {
	...
	case 1:
		...
		while (current->getNext != NULL) {
			if (current->getFirstName == name) {
				printNode(current);
				cout << "Is this your match?\n" << "\t1) YES\n" << "\t2) NO\n";
				
				while (select < 2||select>2)
				{
					cin >> select;
					switch (select) {
					case 1:break;
					case 2: current=current->getNext; break;
					default: cout << "Select a valid option\n";
					}
				}
			}
	...
	case 3:
		current = head;
		while (current->getNext != NULL) {
			if (current->getPhoneNumber == number) {
				printNode(current);
				cout << "Is this your match?\n" << "\t1) YES\n" << "\t2) NO\n";
